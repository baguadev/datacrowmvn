...
public class DiscogsAudioCdSearch extends SearchTask {
    ...
    protected Collection<Object> getItemKeys() throws Exception {
        ...
        String page = HttpConnectionUtil.retrievePage(link);
        ...
        while (idx > -1) {
            ...
            if (!CoreUtilities.isEmpty(number) && !ids.contains(number))
                ids.add(number);
    ...
    private void setYear(DcObject dco, Document document) throws Exception {
        ...
        if (!CoreUtilities.isEmpty(year)) {
            ...
            int fieldIdx = MusicAlbum._C_YEAR;
    ...
    private void setCountry(DcObject dco, Document document) throws Exception {
        ...
        if (!CoreUtilities.isEmpty(country)) {
            DcModule module = DcModules.get(getServer().getModule());
            int fieldIdx = MusicAlbum._F_COUNTRY;
            dco.createReference(fieldIdx, country);
        }
    } 
    
    private void setGenres(DcObject dco, Document document) throws Exception {
        String genres = getValue(document, "Genre:");
        if (!CoreUtilities.isEmpty(genres)) {
            DcModule module = DcModules.get(getServer().getModule());
            StringTokenizer st = new StringTokenizer(genres, ",");
            int fieldIdx = MusicAlbum._G_GENRES;
            while (st.hasMoreElements())
                dco.createReference(fieldIdx, StringUtils.trim((String) st.nextElement()));
        }
    }    
    ...
    private Collection<DcObject> setArtist(DcObject album, Document document) throws Exception {
        ...
        if (artist.length() > 0) {
            ...
            DcModule module = DcModules.get(getServer().getModule());
            while (st.hasMoreElements())
                artists.add(album.createReference(MusicAlbum._F_ARTISTS,
    ...
    private void setWebpage(DcObject dco, String link) {
        DcModule module = DcModules.get(getServer().getModule());
        int fieldIdx = MusicAlbum._N_WEBPAGE;
        ...
        for (int i = 0; i < tracks.getLength(); i++) {
            ...
            if (nLength != null) {
                ...
                if (length.indexOf(":") > -1) {
                    ...
                    try {
                        ...
                        track.setValue(MusicTrack._J_PLAYLENGTH, Long.valueOf(sec.intValue() + (min.intValue() * 60)));
                    } catch (NumberFormatException nfe) {}
                }
            }
            ...
            if (nArtists != null) {
                for (int j = 0; j < nArtists.getLength(); j++) {
                    ...
                    track.createReference(MusicTrack._G_ARTIST, nArtist.getTextContent());
                }
            }
            
            // not a track..
            if (!CoreUtilities.isEmpty(track.getValue(MusicTrack._F_TRACKNUMBER)))
...
//                
//                byte[] image = HttpConnectionUtil.retrieveBytes(nImage.getTextContent());
//                if (!isCancelled()) {
