...
public class MobyGamesSoftwareSearch extends MobyGamesSearch {
    ...
    protected DcObject getItem(URL url, boolean full) throws Exception {
        ...
        if (full) {
            ...
            software.createReference(Software._H_PLATFORM, getInfo(page, "PLATFORM</DIV>", "PLATFORM"));
            software.createReference(Software._K_CATEGORIES, getInfo(page, "GENRE</DIV>", "COREGAMEGENRE"));
    ...
    private void setCompany(DcObject software, String page, String tag1, String tag2, int field) {
        ...
        try {
            ...
            software.createReference(field, company);
    ...
    private void setCovers(DcObject software, String address) throws Exception {
        try {
            String page = HttpConnectionUtil.retrievePage(address + "/cover-art");
    ...
    private void setScreenshots(DcObject software, String ID, String mainPage) throws Exception {
        ...
        try {
            String page = HttpConnectionUtil.retrievePage(getAddress() + link);
            ...
            while (idx > -1 && counter < fields.length && !isCancelled()) {
                ...
                part = HttpConnectionUtil.retrievePage(getAddress() + imagePageUrl);
                ...
                if (screenIdx > -1) {
                    try {
                        ...
                        software.setValue(fields[counter], 
                                new DcImageIcon(HttpConnectionUtil.retrieveBytes(imageUrl)));
                    } catch (Exception e) {
    ...
    private String getImagesLink(String ID, String page) {
        ...
        if (idx > -1) {
            ...
            try {
                HttpConnection conn = HttpConnectionUtil.getConnection(new URL(getAddress() + link));
                if (conn.exists()) {
                    conn.close();
                    return link;
                }
                ...
                conn = HttpConnectionUtil.getConnection(new URL(getAddress() + link));
                if (conn.exists()) {
                    conn.close();
                    return link;
                }
    ...
    private void setMediaImage(DcObject software, String tag, int field, String page) {
        ...
        try {
            ...
            if (idx > -1) {
                ...
                html = HttpConnectionUtil.retrievePage(imagePageUrl);
                ...
                if (idx > -1) {
                    ...
                    byte[] image = HttpConnectionUtil.retrieveBytes(imageUrl);
