            ...
            if (images != null) {
                ...
                for (int i = 0; i < images.getLength() && image < 3; i++) {
                    ...
                    if (!imageIDs.contains(id)) {
                        imageIDs.add(id);
                        byte[] b = HttpConnectionUtil.retrieveBytes("http://sourceforge.net/dbimage.php?id="+ id);
                        if (b != null) {
    ...
    private void setDevelopers(DcObject dco, Document document) throws Exception {
        ...
        if (maintainers != null) {
            for (int i = 0; i < maintainers.getLength(); i++)
                dco.createReference(Software._F_DEVELOPER, maintainers.item(i).getTextContent());
        }
    }
    ...
    private void setCategories(DcObject dco, Document document) throws Exception {
        ...
        if (node != null) {
            ...
            while (st.hasMoreElements()) {
                ...
                dco.createReference(Software._K_CATEGORIES, StringUtils.trim(category));
            }
        }
    }
